...
export async function exportNoti1PdfReact(data: Noti1Data): Promise<void> {
  ...
  const lots = (data.attribution?.lots || []).filter((l: any) => l.numero).map((l: any) => `Lot ${l.numero}`);
  const lotStr = lots.length > 0 ? lots.join('-') : 'Lot 1';
  const titulaireStr = (data.titulaire.denomination || '').replace(/[^a-zA-ZÀ-ÿ0-9 ]/g, '').replace(/\s+/g, ' ').trim().slice(0, 30);
  const fileName = `${prefix}_${lotStr}_${titulaireStr}_NOTI 1.pdf`;
