...
import { generateNoti5Html } from '../utils/noti5HtmlGenerator';
import type { Noti5Data } from '../types';

interface Noti5ViewerProps {
  data: Noti5Data;
  onClose: () => void;
}

export default function Noti5Viewer({ data, onClose }: Noti5ViewerProps) {
  const [htmlContent, setHtmlContent] = useState<string>('');

  useEffect(() => {
    generateNoti5Html(data).then(setHtmlContent);
  }, [data]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full h-full max-w-6xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            Aperçu NOTI5 - Notification du marché public
          </h2>
          <button
            onClick={onClose}
