...
import { generateNoti3Html } from '../utils/noti3HtmlGenerator';
import type { Noti3Data } from '../types';

interface Noti3ViewerProps {
  data: Noti3Data;
  onClose: () => void;
}

export default function Noti3Viewer({ data, onClose }: Noti3ViewerProps) {
  const [htmlContent, setHtmlContent] = useState<string>('');

  useEffect(() => {
    generateNoti3Html(data).then(setHtmlContent);
  }, [data]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full h-full max-w-6xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            Aperçu NOTI3 - Notification de rejet
          </h2>
          <button
            onClick={onClose}
