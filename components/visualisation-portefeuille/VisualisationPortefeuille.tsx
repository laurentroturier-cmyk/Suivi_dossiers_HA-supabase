...
import { Upload, BarChart2, RefreshCw, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
function rawToDbRow(raw: RawElement, existing?: PortfolioElement): DbRow {
  return {
    ...
    // Préserver les annotations existantes si déjà en base
    display_name: existing?.displayName ?? null,
    montant_k:    existing?.montant ?? null,
  };
}

// ─── Composant principal ─────────────────────────────────────────────────────────

export function VisualisationPortefeuille(): React.ReactElement {
  ...
  const handleLoad = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = async (ev) => {
        ...
        try {
          // Récupérer les annotations existantes pour les préserver
          const existingMap: Record<number, PortfolioElement> = {};
          elements.forEach(el => { existingMap[el.id] = el; });

          // Construire les lignes DB
          const rows: DbRow[] = rawElements.map(raw =>
            rawToDbRow(raw, existingMap[raw.id])
          );
          ...
          // Recharger depuis la base pour avoir l'état cohérent
  ...
  const filtered = elements.filter(el => {
    ...
    if (filterAcheteur && el.acheteur !== filterAcheteur) return false;
    return true;
  });
      ...
      <div className="flex items-center justify-between px-6 py-4" style={{ backgroundColor: '#2F5B58' }}>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {importing ? 'Import en cours…' : 'Charger Excel'}
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <aside className="w-64 flex-shrink-0 flex flex-col bg-white border-r border-gray-100 overflow-hidden">
            <div className="p-3 border-b border-gray-100 space-y-2">
              ...
              <p className="text-xs text-gray-400 text-right">{filtered.length} / {elements.length}</p>
            </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    {selectedEl.segment && <p className="text-xs text-gray-500">{selectedEl.segment}</p>}
                  </div>
